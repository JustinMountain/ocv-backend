  ...
  for (const oneMember of axiosResponse) {
    ...
    if (selector('#contact .row .col-md-3 > h4').text() === 'Hill Office') {
      ...
      if (contactNumbers) {
        ...
        if (phoneSplit[0]) {
          const phoneparts = phoneSplit[0].split(':');
          if (phoneparts.length > 1 && phoneparts[1]) {
            phoneNumber = phoneparts[1].trim();
          }
        }
        if (phoneSplit[1]) {
          const faxparts = phoneSplit[1].split(':');
          if (faxparts.length > 1 && faxparts[1]) {
            faxNumber = faxparts[1].trim();
          }
        }
      }
